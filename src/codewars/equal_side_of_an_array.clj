...
(ns codewars.equal-side-of-an-array)
...
(defn find-even-index
  ...
  (loop [index 0]
    (if (>= index (count arr))
      -1
      (let [first-half (take index arr)
            second-half (drop (inc index) arr)]
        (println first-half " " second-half)
        (if (= (reduce + first-half) (reduce + second-half))
          index
          (recur (inc index)))))))
