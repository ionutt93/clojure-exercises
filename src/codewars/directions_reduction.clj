...
; returns nil when the path is reduced to nothing. The Rust version takes a slice of enum Direction {NORTH, SOUTH, EAST, WEST}.
;
;Examples
;
...
; "WEST" and "SOUTH", "SOUTH" and "EAST" are not directly opposite of each other and can't become such. Hence the result path is itself : ["NORTH", "WEST", "SOUTH", "EAST"].

(ns codewars.directions-reduction)
...
(defn add-dir
  [current-arr next]
  (if (= (first current-arr) (get opposite-dir next))
    (drop 1 current-arr)
    (conj current-arr next)))

(defn dirReduc
  [arr]
  (not-empty (reverse (reduce add-dir '() arr))))
